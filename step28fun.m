...
% These matrices are the stacked block diagonal matrices
A = A(1).merged;
B1 = B(1).merged1;
B2 = B(1).merged2;
B3 = B(1).merged3;
...
%Extract the correct indices, so that the right values of P_cons can be set
%to 1 or minus 1.
for i = 1:Np
...
%These are the constraints on p, for j>0
for i = 2:(Np)
...
%constraints on dumy variable p, given some initial condition (so j=0)
...
%% Gurobi
%Declare the variable type, and the (in)equality
vtype = '';
sense = '';
...
%The P_imp constraints are equality constraints
for i=1:Np
   sense = strcat(sense,'='); 
end
